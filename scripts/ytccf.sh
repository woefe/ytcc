...
# ytcc 5 The YouTube channel checker
...
if [[ $THUMBNAILS -eq 1 ]]; then
    ...
    if [[ $TERM == "xterm-kitty" ]]; then
        function draw_preview {
            ...
            kitten icat --clear --stdin no --transfer-mode file --z-index -1 --unicode-placeholder --place "${COLUMNS}x${LINES}@${X}x${Y}" --scale-up "${@}" > /dev/tty
        }
    else
        check_cmd ueberzug
